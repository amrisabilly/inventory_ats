...
class PermintaanProduksiController extends Controller
{
    ...
    public function data()
    {
        return DataTables::of(PermintaanProduksi::with(['produk', 'user']))
            ...
            ->addColumn('aksi', fn(PermintaanProduksi $row) => view('partials.datatables.actions', ['showUrl' => route('permintaan-produksi.show', $row)])->render())
    ...
    public function show(PermintaanProduksi $permintaanProduksi): View
    {
        return view('permintaan-produksi.show', compact('permintaanProduksi'));
    }

    public function proses(PermintaanProduksi $permintaanProduksi, ProduksiService $service): RedirectResponse
    {
        $service->prosesPermintaanProduksi($permintaanProduksi);

        return back()->with('success', 'Permintaan produksi berhasil diproses.');
    }
