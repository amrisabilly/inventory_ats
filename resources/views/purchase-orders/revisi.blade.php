...
@section('content')
    <x-card title="Revisi {{ $purchaseOrder->nomor_po }}">
        ...
        <form method="POST" action="{{ route('purchase-orders.revisi', $purchaseOrder) }}" class="max-w-3xl space-y-5 js-repeater">
            @csrf
            @method('PUT')
            <div>
                <label class="mb-1 block text-sm font-medium">Tanggal PO</label>
                <input class="form-input" type="date" name="tanggal_po" value="{{ old('tanggal_po', $purchaseOrder->tanggal_po?->toDateString()) }}" required>
            ...
            <div class="border-t border-border pt-4">
                <div class="mb-3 flex items-center justify-between">
                    ...
                    <button type="button" class="js-add-row text-sm font-medium text-primary hover:underline">+ Tambah material</button>
                </div>
                        ...
                        <div class="js-repeater-row grid gap-3 sm:grid-cols-[1fr_10rem_auto]">
                            <select class="form-input" name="details[{{ $index }}][material_id]" required>
                                ...
                                @foreach ($materials as $material)
                                    <option value="{{ $material->id }}" @selected((string) ($detail['material_id'] ?? '') === (string) $material->id)>{{ $material->nama_material }}</option>
                                @endforeach
                            </select>
                            @php($minimumJumlah = $purchaseOrder->detailPos->firstWhere('material_id', (int) ($detail['material_id'] ?? 0))?->jumlah_material ?? 1)
                            <input class="form-input" type="number" min="{{ $minimumJumlah }}" name="details[{{ $index }}][jumlah_material]" value="{{ $detail['jumlah_material'] ?? $minimumJumlah }}" required>
                            <x-field-error field="details.{{ $index }}.jumlah_material" />
                            <button type="button" class="js-remove-row rounded-lg px-3 text-sm text-danger hover:bg-red-50">Hapus</button>
                        </div>
                    @endforeach
                </div>
                <template class="js-repeater-template"><div class="js-repeater-row grid gap-3 sm:grid-cols-[1fr_10rem_auto]"><select class="form-input" name="details[__INDEX__][material_id]" required><option value="">Pilih material</option>@foreach ($materials as $material)<option value="{{ $material->id }}">{{ $material->nama_material }}</option>@endforeach</select><input class="form-input" type="number" min="1" name="details[__INDEX__][jumlah_material]" value="1" required><button type="button" class="js-remove-row rounded-lg px-3 text-sm text-danger hover:bg-red-50">Hapus</button></div></template>
